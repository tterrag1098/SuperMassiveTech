...
public class ConfigHandler
{
    ...
    public static void init(File file)
    {
        ...
        strength = (float) config.get(section, "gravityStrength", 0.2,
        ...
        maxGravityXZ = (float) config.get(section, "maxGravityXZ", 0.1,
                "The max gravity that can be applied in the X+Z directions (prevents spikes)").getDouble(0.1);
        maxGravityY = (float) config
                .get(section, "maxGravityY", 0.028,
                ...
                .getDouble(0.028);
        minGravity = (float) config.get(section, "minGravity", 0,
                "The minimun force to apply, can be zero. Used to prevent \"wobbling\" near the center of axes").getDouble(0);

        gravArmorDrain = config.get(section, "gravArmorDrain", 100, "The base value that gravity effects passively drain gravity armor")
                .getInt();
        ...
        showOredictTooltips = config.get("other", "showOredictionaryTooltip", false,
        ...
        fieldIgnorePlayers = config.get("armor", "fieldIgnorePlayers", fieldIgnorePlayers,
                "Whether or not the anti-grav field ignores players.").getBoolean(fieldIgnorePlayers);
